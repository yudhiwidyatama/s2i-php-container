/* @(#) $Id: //bas/640_REL/src/include/sapuc.h#68 $ SAP*/
/* CCQ_CCU_FILE_OFF */

#ifndef SAPUC_H
#define SAPUC_H "$Id: //bas/640_REL/src/include/sapuc.h#68 $"

/*******************************************************************************
 * (c) Copyright SAP AG, Walldorf, 1996 - 2007
 *
 *
 ...
 * 19.02.2008 mit      In 7.11 (BIN): 22.08.2007 mau add MAX_PATH_LN for SAPonNT
 * 23.03.2007 que      link libsapu16 content statically into portlib
 * 28.07.2006 strehle  added mkstempU and mkstempsU
 * 17.01.2006 kkr      Add SAPwithICU_BREAK
 * 10.11.2005 bs       ato*U16(): map it correctly to U16 functions also in the
 *                       non-Unicode case
 * 18.04.2005 bs       Make SAP_U2 and SAP_UTF16 identical
 * 04.02.2005 bs       remove strerror_s functions and getenv_sRFB
 * 17.09.2004 meb      downport secure lib to 6.40
 * 24.05.2004 cla      add low-surrogate check in mblenU
 ...
 * 27.04.2001 hm       RFC-SDK only: add conversion wchar_t <> SAP_U.
...
#define iU16(par)      iU16_HELP(par) /* ... resolved before the 'L' is added */
  ...
  #elif defined(SAPwithoutUTF16_LITERALS) && !defined(SAPccQ)
 ...
 *---------------------------------------------------------------------------*/
  ...
  #define SAP_UNAME_MEM_RET(name)    (SAP_CHAR*)name
...
/* SAP_U_R_PROTOTYPE: prototype of R function, equivalent to SAP_U16_PROTOTYPE */
#define SAP_R_PROTOTYPE(name)  externC name##R_RETURN name##R name##R_TPARAMS;
...
#define strcasecmpR_RETURN              int
#define strcasecmpR_TPARAMS             (const char* s1, const char* s2)
#define strcasecmpR_PARAMS              (s1, s2)
SAP_R_PROTOTYPE(strcasecmp)
...
#define strncasecmpR_RETURN             int
#define strncasecmpR_TPARAMS            (const char* s1, const char* s2, size_t n)
#define strncasecmpR_PARAMS             (s1, s2,n)
SAP_R_PROTOTYPE(strncasecmp)
...
#define strtoupperU16_RETURN            SAP_UTF16*
#define strtoupperU16_TPARAMS           (SAP_UTF16 * s1)
#define strtoupperU16_PARAMS            (s1)
SAP_U16_PROTOTYPE(strtoupper)
#define strtoupperR_RETURN              char*
#define strtoupperR_TPARAMS             (char * s1)
#define strtoupperR_PARAMS              (s1)
SAP_R_PROTOTYPE(strtoupper)
#define strtoupperU(s1)                 SAP_UNAME_UR(strtoupper)(s1)

#define strtolowerU16_RETURN            SAP_UTF16*
#define strtolowerU16_TPARAMS           (SAP_UTF16 * s1)
#define strtolowerU16_PARAMS            (s1)
SAP_U16_PROTOTYPE(strtolower)
#define strtolowerR_RETURN              char*
#define strtolowerR_TPARAMS             (char * s1)
#define strtolowerR_PARAMS              (s1)
SAP_R_PROTOTYPE(strtolower)
#define strtolowerU(s1)                 SAP_UNAME_UR(strtolower)(s1)

#define strntoupperU16_RETURN           SAP_UTF16*
#define strntoupperU16_TPARAMS          (SAP_UTF16 * s1, size_tU len)
#define strntoupperU16_PARAMS           (s1,len)
SAP_U16_PROTOTYPE(strntoupper)
#define strntoupperR_RETURN             char*
#define strntoupperR_TPARAMS            (char * s1, size_t len)
#define strntoupperR_PARAMS             (s1,len)
SAP_R_PROTOTYPE(strntoupper)
#define strntoupperU(s1,len)            SAP_UNAME_UR(strntoupper)(s1,len)

#define strntolowerU16_RETURN           SAP_UTF16*
#define strntolowerU16_TPARAMS          (SAP_UTF16 * s1, size_tU len)
#define strntolowerU16_PARAMS           (s1,len)
SAP_U16_PROTOTYPE(strntolower)
#define strntolowerR_RETURN             char*
#define strntolowerR_TPARAMS            (char * s1, size_t len)
#define strntolowerR_PARAMS             (s1,len)
SAP_R_PROTOTYPE(strntolower)
#define strntolowerU(s1,len)            SAP_UNAME_UR(strntolower)(s1,len)

#define strcpytoupperU16_RETURN         SAP_UTF16*
#define strcpytoupperU16_TPARAMS        (SAP_UTF16 * dst, const SAP_UTF16 * src)
#define strcpytoupperU16_PARAMS         (dst,src)
SAP_U16_PROTOTYPE(strcpytoupper)
#define strcpytoupperR_RETURN           char*
#define strcpytoupperR_TPARAMS          (char * dst, const char * src)
#define strcpytoupperR_PARAMS           (dst,src)
SAP_R_PROTOTYPE(strcpytoupper)
#define strcpytoupperU(dst,src)         SAP_UNAME_UR(strcpytoupper)(dst,src)

#define strcpytolowerU16_RETURN         SAP_UTF16*
#define strcpytolowerU16_TPARAMS        (SAP_UTF16 * dst, const SAP_UTF16 * src )
#define strcpytolowerU16_PARAMS         (dst,src)
SAP_U16_PROTOTYPE(strcpytolower)
#define strcpytolowerR_RETURN           char*
#define strcpytolowerR_TPARAMS          (char * dst, const char * src)
#define strcpytolowerR_PARAMS           (dst,src)
SAP_R_PROTOTYPE(strcpytolower)
...
#define _getcwdU(buf, size)            getcwdU(buf, size)
...
#define printfU16_TPARAMS              (const SAP_UTF16 *format, ...) /* PRINTFLIKE1 */
#define printfU16_PARAMS               (format, rest_args)
SAP_U16_PROTOTYPE_STDC(printf)
#define printfU                        SAP_UNAME(printf)

#define fprintfU16_RETURN              int
#define fprintfU16_TPARAMS             (FILE* s, const SAP_UTF16 *format, ...) /* PRINTFLIKE2 */
#define fprintfU16_PARAMS              (s, format, rest_args)
SAP_U16_PROTOTYPE_STDC(fprintf)
#define fprintfU                       SAP_UNAME(fprintf)

#define sprintfU16_RETURN              intU
#define sprintfU16_TPARAMS             (SAP_UTF16* s, const SAP_UTF16 *format, ...) /* PRINTFLIKE2 */
#define sprintfU16_PARAMS              (s, format, rest_args)
SAP_U16_PROTOTYPE_STDC(sprintf)
#define sprintfU                       SAP_UNAME(sprintf)

#define snprintfU16_RETURN             intU
#define snprintfU16_TPARAMS            (SAP_UTF16* s, size_t n, const SAP_UTF16 *format, ...) /* PRINTFLIKE3 */
#define snprintfU16_PARAMS             (s, n, format, rest_args)
SAP_U16_PROTOTYPE_STDC(snprintf)
...
#define sscanfU16_TPARAMS              (const SAP_UTF16 *s, const SAP_UTF16 *format, ... ) /* SCANFLIKE2 */
#define sscanfU16_PARAMS               (s, format, rest_args )
SAP_U16_PROTOTYPE_STDC(sscanf)
#define sscanfU                        SAP_UNAME(sscanf)

/* deprecated */
#define scanfU16_RETURN                int
#define scanfU16_TPARAMS               (const SAP_UTF16 *format, ... ) /* SCANFLIKE1 */
#define scanfU16_PARAMS                (format, rest_args )
SAP_U16_PROTOTYPE_STDC(scanf)
#define scanfU                         SAP_UNAME(scanf)

/* deprecated */
#define fscanfU16_RETURN               int
#define fscanfU16_TPARAMS              (FILE *s, const SAP_UTF16 *format, ... ) /* SCANFLIKE2 */
#define fscanfU16_PARAMS               (s, format, rest_args )
SAP_U16_PROTOTYPE_STDC(fscanf)
...
/* Although EBCDIC vsnprintf is supported by the OS beginning with V5R2,
   we use our implementation for the RFCSDK build which should also support V5R1.
   The kernel with minimum Release V5R2 will use the OS Version for EBCDIC and
   only use our implementation for ASCII.
*/   
#if defined SAPonOS400  && !defined(SAPwithCHAR_EBCDIC)
  externC intU as4_snprintfA
      ( char* buffer, size_tR count, const char* format, ... );
  externC intU as4_vsnprintfA
...
#define fget_strR_RETURN                 int
#define fget_strR_TPARAMS                (char *s, int n, FILE *stream, \
                                          SAP_UC repl_char)
#define fget_strR_PARAMS                 (s, n, stream, repl_char)
SAP_R_PROTOTYPE(fget_str)
...
#define fget_lineR_RETURN                int
#define fget_lineR_TPARAMS               (char *s, int n, FILE *stream, \
                                          SAP_UC repl_char)
#define fget_lineR_PARAMS                (s, n, stream, repl_char)
SAP_R_PROTOTYPE(fget_line)
...
#define fget_longR_RETURN                int
#define fget_longR_TPARAMS               (long int* ptr, FILE *stream)
#define fget_longR_PARAMS                (ptr, stream)
SAP_R_PROTOTYPE(fget_long)
...
#define fget_intR_RETURN                 int
#define fget_intR_TPARAMS                (int* ptr, FILE *stream)
#define fget_intR_PARAMS                 (ptr, stream)
SAP_R_PROTOTYPE(fget_int)
...
#if !defined(SAPUCX_H)
externC void CDCL_U nlsui_store_cmdline( int argc, SAP_UC *argv[] );
...
#define isxxxNUC(isfctnuc,c)   ( sizeofR(c) == sizeofR(SAP_CHAR) ?  \
 ...
 #define isxdigitU(c)       isxxxNUC(isxdigit,c)
#else /* Now SAPwithUNICODE */
 ...
 *----------------------------------------------------------------------------*/
  ...
  #define lstatU(path, buffer)            SAP_UNAME_(lstat)(path, buffer)
    ...
    #define fstatU16_RETURN               int
    #define fstatU16_TPARAMS              (int filedes, struct stat_U16 *buffer)
    #define fstatU16_PARAMS               (filedes,buffer)
    SAP_U16_PROTOTYPE(fstat)
...
/*-----------------------------------------------------------------------------
 * sapuc.h  -   defines for sap_readline() (you'll need to link dllib$(LIBE))
 *----------------------------------------------------------------------------*/

void sap_readline_possible_commands( const char *possible_commands );

char *sap_readlineR(const char *prompt, char *line, const SAP_UINT maxchars);

#if defined(SAPwithUNICODE)
SAP_UC *sap_readlineU(const SAP_UC *prompt, SAP_UC *line, const SAP_UINT maxchars);
#else
#define sap_readlineU sap_readlineR
#endif

/*-----------------------------------------------------------------------------
 * sapuc.h  -   end of sap_readline()
 *----------------------------------------------------------------------------*/
...
/*sprintf_s*/
externC int sprintf_sRFB(char * s, size_tR s1max,const char *format, ...);
...
BEGIN_NS_STD_C_HEADER
  SAP_U16_PROTOTYPE_FLIKE(sprintf_s)    /* PRINTFLIKE2 */;
END_NS_STD_C_HEADER
#define sprintf_sU                              SAP_UNAME_UR(sprintf_s)

/*snprintf_s*/
externC int snprintf_sRFB(char * s, size_tR n,const char *format, ...);
...
BEGIN_NS_STD_C_HEADER
  SAP_U16_PROTOTYPE_FLIKE(snprintf_s)    /* PRINTFLIKE2 */;
END_NS_STD_C_HEADER
